package com.coolerpromc.unstriplog.client;
...
public record BarkItemRenderer() implements SpecialModelRenderer<BarkTypeConfig.BarkTypeEntry> {
    ...
    public void submit(BarkTypeConfig.@Nullable BarkTypeEntry barkTypeEntry, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, int overlayCoords, boolean hasFoil, int outlineColor) {
        Identifier texture = Constants.id("textures/item/bark.png");
    ...
    private static TextureShape loadTextureShape(Identifier texture) {
        try (NativeImage image = NativeImage.read(Minecraft.getInstance().getResourceManager().open(texture))) {
            ...
            Constants.LOGGER.warn("Failed to read bark texture {} for generated outline rendering, using a solid fallback", texture, e);
    ...
    public BarkTypeConfig.@Nullable BarkTypeEntry extractArgument(ItemStack itemStack) {
        return itemStack.get(ModDataComponents.BARK_TYPE.get());
    }
