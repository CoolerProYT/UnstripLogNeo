        ...
        event.enqueueWork(() -> {
            for (Block block : BuiltInRegistries.BLOCK) {
                try {
                    ...
                    if (strippable != null){
                        LOGS.add(block);
                        STRIPPED_LOG.put(strippable.strippedBlock(), block);
                    }
                }
            }
            ...
            LOGS.forEach(block -> {
                if(!block.builtInRegistryHolder().getKey().identifier().getNamespace().equals("minecraft")) return;
                String name = block.builtInRegistryHolder().getKey().identifier().getPath().replace("_wood", "").replace("_log", "").replace("_block", "").replace("_stem", "").replace("_hyphae", "");
